...
def test_mask_all_nodes():
    ...
    data = GraphData(
        torch.randn(5, 5), torch.randn(3, 2), torch.randn(1, 1),
...
def test_mask_one_edges():
    ...
    expected_edges = torch.LongTensor([
        [0, 0],
        [1, 3]
    ])
    ...
    data = GraphData(
        torch.randn(5, 5), e, torch.randn(1, 1), edges=edges
    )
...
def test_mask_one_node():
    ...
    data = GraphData(
        x, e, torch.randn(1, 1), edges=edges
    )
