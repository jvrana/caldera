    ...
    def __new__(cls, clsname, superclasses, attributedict):
        ...
        if has_new_fields:
            if '__init__' not in attributedict:
                raise ValueError("__init__ must be defined if any {} or {} are defined".format(
        ...
        if has_new_fields:
            ...
            for field_name, field in fields:
                if field_name not in argspec.args:
                    raise ValueError(
                        "field '{}: {}' is missing from __init__ signature".format(field_name, field.annotation))
                elif field_name in argspec.annotations and argspec.annotations[field_name] != field.annotation:
                    raise ValueError(
...
class MyComposition(TensorComposition):
    ...
    def __init__(self, node: torch.FloatTensor, edge: torch.FloatTensor, edges: torch.LongTensor):
        self.node = node
        self.edge = edge
        self.edges = edges
