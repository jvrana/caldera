...
class Encoder(NNGraph):
    ...
    def forward(self, data: GraphBatch):
        ...
        return GraphBatch(x, e, g, edges=data.edges, node_idx=data.node_idx, edge_idx=data.edge_idx)
...
class Core(NNGraph):
    ...
    def __init__(self, config: NetConfig):
        super().__init__()
        config = config.core
        self.add_node(gnn.Flex(gnn.Dense)(..., *[config.node.size]*config.node.depth), 'node')
        self.add_node(gnn.Flex(gnn.Dense)(..., *[config.edge.size]*config.edge.depth), 'edge')
        self.add_node(gnn.Flex(gnn.Dense)(..., *[config.glob.size]*config.glob.depth), 'glob')
        self.add_edge(lambda data: data.x, 'node')
        self.add_edge(lambda data: data.e, 'edge')
        self.add_edge(lambda data: data.g, 'glob')
