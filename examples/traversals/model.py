...
class Network(torch.nn.Module):
    ...
    def init_encoder(self) -> GraphEncoder:
        return GraphEncoder(
            edge_block=EdgeBlockEncoder(
                self.config.edge_encode.size, self.config.edge_encode.dropout
            ),
            node_block=NodeBlockEncoder(
                self.config.node_encode.size, self.config.node_encode.dropout
            ),
            global_block=GlobalBlockEncoder(
                self.config.glob_encode.size, self.config.glob_encode.dropout
            ),
        )
    ...
    def init_decoder(self) -> GraphEncoder:
        return GraphEncoder(
            edge_block=EdgeBlockEncoder(
                self.config.edge_decode.size, self.config.edge_decode.dropout
            ),
            node_block=NodeBlockEncoder(
                self.config.node_decode.size, self.config.node_decode.dropout
            ),
            global_block=GlobalBlockEncoder(
                self.config.glob_decode.size, self.config.glob_decode.dropout
            ),
        )
